...
describe('computeDurationMs', () => {
  it('stays within the configured bounds', () => {
    expect(computeDurationMs(1)).toBeGreaterThanOrEqual(WHEEL_CONFIG.minDurationMs)
    expect(computeDurationMs(1000)).toBeLessThanOrEqual(WHEEL_CONFIG.maxDurationMs)
  })

  it('grows with pool size', () => {
    expect(computeDurationMs(50)).toBeGreaterThan(computeDurationMs(2))
  })
})
...
describe('buildReelSequence', () => {
  ...
  it('produces a deterministic sequence for a deterministic rng', () => {
    const rng = () => 0
    const a = buildReelSequence(pool, winner, rng)
    const b = buildReelSequence(pool, winner, rng)
    expect(a).toEqual(b)
  })

  it('handles a single-eligible-participant pool without crashing', () => {
    const solo = makePool(['OnlyOne'])
    const sequence = buildReelSequence(solo, solo[0])
  ...
  it('sets duration from computeDurationMs for the given pool size', () => {
    const sequence = buildReelSequence(pool, winner, () => 0)
    expect(sequence.durationMs).toBe(computeDurationMs(pool.length))
  })
})
